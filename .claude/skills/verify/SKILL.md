...
`.env.local` already exists locally with Supabase + AI credentials. Then exercise the affected route:
...
- To check a protected page actually renders, log in through the browser (or use the E2E path below). Users without a `user_preferences` row get forced to `/onboarding` — a redirect there is middleware behavior, not a bug in your change.
...
- Supabase "missing table" errors (`isMissingTableError` in `lib/supabase/errors.ts`) mean migrations aren't applied to the target project — see the `db-migration` skill.
...
- Needs network access to the live Supabase Auth endpoint; a browser-side `Failed to fetch` at login is a network problem, not an app regression (see `PRODUCTION_NOTES.md`).
