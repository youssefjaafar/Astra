...
import { prepareCached } from "@/lib/db/sqlite/connection";
...
export function createLocalUser(
  ...
  const existing = prepareCached("SELECT id FROM auth_users WHERE email = ?").get(normalizedEmail);
...
export function verifyLocalCredentials(
  ...
  const row = prepareCached("SELECT * FROM auth_users WHERE email = ?").get(
...
export function createLocalSession(userId: string): string {
  ...
  prepareCached("DELETE FROM auth_sessions WHERE expires_at < ?").run(new Date().toISOString());
  prepareCached("INSERT INTO auth_sessions (token_hash, user_id, expires_at) VALUES (?, ?, ?)").run(
    hashToken(token),
    userId,
    expiresAt,
  );
...
export function deleteLocalSession(token: string | undefined | null): void {
  if (!token) return;
  prepareCached("DELETE FROM auth_sessions WHERE token_hash = ?").run(hashToken(token));
}
...
export function userHasLocalPreferences(userId: string): boolean {
  return Boolean(prepareCached("SELECT id FROM user_preferences WHERE user_id = ?").get(userId));
}
